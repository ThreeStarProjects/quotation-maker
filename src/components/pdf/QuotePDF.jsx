...
const S = StyleSheet.create({
  ...
  // Inline image cell
  imgCell:    { padding:'4pt 4pt', flexDirection:'row', flexWrap:'wrap', gap:3, alignItems:'flex-start' },
  imgThumb:   { width:40, height:40, objectFit:'cover', borderRadius:2, border:'0.5pt solid #2C7DA0' },
  noImg:      { fontSize:7, color:'#ccc', fontFamily:'Helvetica-Oblique', padding:'4pt 4pt' },
  // Totals
...
// Columns — with images column, desc is narrowed from 38% to 28%, images gets 10%
// Without images, desc stays at 38% and no images column shown
const COLS_NO_IMG = [
...
const COLS_WITH_IMG = [
  { key:'sr',    label:'SR',          w:'5%',  align:'center' },
  { key:'name',  label:'PRODUCT',     w:'18%', align:'left'   },
  { key:'desc',  label:'DESCRIPTION', w:'28%', align:'left'   },
  { key:'unit',  label:'UNIT',        w:'7%',  align:'center' },
  { key:'qty',   label:'QTY',         w:'5%',  align:'center' },
  { key:'rate',  label:'UNIT RATE',   w:'12%', align:'right'  },
  { key:'total', label:'TOTAL',       w:'13%', align:'right'  },
  { key:'imgs',  label:'REF. IMAGES', w:'12%', align:'left'   },
]
...
export default function QuotePDF({ co, cl, items, charges, gst, gstType, totals, note, terms, bank, logo }) {
  ...
  // Only show images column if at least one item has images
  const anyImages = items.some(it => it.images && it.images.length > 0)
  const COLS = anyImages ? COLS_WITH_IMG : COLS_NO_IMG
          ...
          {items.length === 0
            ...
            : items.map((it, idx) => (
              <View key={it.id} style={[S.tRow, { backgroundColor: idx % 2 === 0 ? '#fff' : '#f5f8fc' }]}>
                ...
                <Text style={[S.tCell, { width: anyImages ? '18%' : '20%', fontFamily:'Helvetica-Bold', color:'#1F4E79' }]}>{it.name}</Text>
                <Text style={[S.tCell, { width: anyImages ? '28%' : '38%', fontSize:7.5, color:'#444', lineHeight:1.5 }]}>{it.desc}</Text>
                ...
                {/* Images column — only rendered when any item has images */}
                {anyImages && (
                  it.images && it.images.length > 0
                    ? (
                      <View style={[S.imgCell, { width:'12%' }]}>
                        {it.images.map((img, i) => (
                          <Image key={i} src={img.src} style={S.imgThumb} />
                        ))}
                      </View>
                    )
                    : <Text style={[S.noImg, { width:'12%' }]}>-</Text>
                )}
              </View>
            ))
          }
